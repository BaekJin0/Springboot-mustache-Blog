...
public class UserController {
    ...
    // 컴포지션(의존성 연결) : 컨트롤러는 레파지토리에 의존해야 해!
    private final UserService userService;
    private final HttpSession session;

    // DB에 username 확인
    // user의 username이 동일한지 확인해줄래? - 응답은 무조건 json
    // 부득이하게 예외적으로 동사사용할 때도 있다.
    // get만으로 동사가 부족하기 때문이다.
    ...
    @PostMapping("/join")
    public String join(User user) { // 행위, 페이지 아니다.
    ...
    public String login(User user, HttpServletResponse response) {
        ...
        // 쿠키에 JSESSIONID를 85로 가져오면 session의 자기 공간을 가리킴
        ...
        if (userEntity != null) {
            ...
            if (user.getRemember() != null && user.getRemember().equals("on")) {
                // F12 Application Cookies 프로토콜이라서 저장한거임!! redirection과 상관 없다! 브라우저가 저장시킨다
                response.addHeader("Set-Cookie", "remember=" + userEntity.getUsername()); // 프로토콜에 없는 http 헤더 키값 만들어낸 것

                // response.addHeader("Set-Cookie", "hi=hihihihi;"); // 프로토콜에 없는 http 헤더 키값 만들어낸
                // 것
                // response.addCookie(cookie); // 프로토콜에 없는 http 헤더 키값 만들어낸 것

                // response.setHeader("hello", "안녕");
                // F12 Network Header responseheader에 남는데 얘는 redirect되어서 request 사라진다.
            }
