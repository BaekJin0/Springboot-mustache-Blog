...
public class PostController {
    ...
    @GetMapping({ "/", "post/list" }) // { "/", "post/list" }로 쓰면 두 가지 방법으로 들어올 수 있다.
    ...
    public String detail(@PathVariable Integer id, Model model) { // int는 null이 없음, 초기값이 0 // Integer는 초기값이 null
    ...
    public String updateForm(@PathVariable Integer id, Model model) {
        ...
        return "post/updateForm"; // ViewResolver 도움 받는다.
    }
    ...
    public String post(Post post) {
        ...
        // id가 필요한데 오브젝트를 넣어도 된다 → 알아서 id(PK)만 뽑아간다
        // 오브젝트 안넣으면 FK연결 안 된다
        User principal = (User) session.getAttribute("principal");
