...
public class User {
    ...
    @Column(length = 20, unique = true) // VARCHAR(20) 길이 설정
    ...
    // 시간 → DB는 LocalDateTime 타입이 없으니 알아서 datetime 타입으로 바뀐다
    // 자바에선 커멜표기법인데 디비는 언더바가 디폴트다
    // 내가 설정한 이름 그대로 설정되게 하는 기능이 있다
    ...
    private String remember; // 이렇게 하면 테이블이 만들어진다. 필요없는데.
}